...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend.vercel.app/movies')
      ...
      .then(data => {
        ...
        // let shortOver = data.movies.map(desc => ({
        //   desc: desc.overview
        ...
        // // for (let shorter of shortOver) {

        // //   console.log(shorter.desc.length)
        // //   if (shorter.desc.length > 250) {
        // //    newShortOver = shorter.desc.substring(0, 250) + '...';
        // //  }
        // // }

        // for (let i = 0; i < shortOver.length; i++) {
        //   console.log(shortOver[i].desc.length)
        //   if (shortOver[i].desc.length > 250) {
        //     newShortOver = shortOver[i].desc.substring(0, 250) + '...';
        //   }
...
const shortOverview = moviesList.map(desc => (
{
...
// for (let shorts of shortOverview) {
//   if (shorts.overview.length > 250) {
//     finalShorts.push(shorts.overview.substring(0, 250) + '...');
//   }
...
const newMoviz = moviesList.map(movies => (
  {
  title: movies.title,
  poster: 'https://www.themoviedb.org/t/p/w600_and_h900_bestv2' + movies.poster_path,
  voteAverage: movies.vote_average,
  voteCount: movies.vote_count,
  overview: movies.overview.length > 250 ? movies.overview.substring(0, 250) + '...' : movies.overview
...
// const newMoviesData = moviesList.map((data, i) => {
...
// })
  ...
  const movies = newMoviz.map((data, i) => {
